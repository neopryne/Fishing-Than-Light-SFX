...
local function playSoundInternal(self, soundObject, channelQueue)
    ...
    if soundObject.loop then
        self:queueSound(channelQueue.id, soundObject.name, soundObject.volume, soundObject.loop, soundObject.duration, soundObject.onPlay)
    end
...
---@param onPlay function|nil zero argument function to be called when the sound begins
---@return number channel of the sound.  Not the one hyperspace uses.
function SoundManager:queueSound(channelId, soundName, volume, looping, duration, onPlay)
    ...
    self._channelQueues[channelId].queue:push_right({name=soundName, volume=volume, loop=looping, duration=duration, onPlay=onPlay})
    return channelId
end
...
---@param onPlay function|nil zero argument function to be called when the sound begins
---@return number channel of the sound.  Not the one hyperspace uses.
function SoundManager:playSound(channelId, soundName, volume, looping, duration, onPlay)
    self:emptyQueue(channelId)
    return self:queueSound(channelId, soundName, volume, looping, duration, onPlay)
end
...
---Stops the current sound on a channel and goes to the next one.  TODO wrong behavior when called twice in a row.  No way to cancel a looping sound, or know if the upcoming sound is a looping sound that is playing or hasn't yet played.
